...
class AdminStudentView extends React.Component {
    ...
    firstPage =()=> {
        ...
        if(this.state.currentPage>firstPage){
            this.getAllStudentUser(firstPage);
        }
    }
    prevPage =()=> {
        let prevPage=1;
        if(this.state.currentPage>prevPage){
            this.getAllStudentUser(this.state.currentPage - prevPage)
        }
    }
    ...
    lastPage =()=> {
        ...
        if(this.state.currentPage<Math.ceil(this.state.totalElements/this.state.usersPerPage)){
            this.getAllStudentUser(Math.ceil(this.state.totalElements/this.state.usersPerPage));
        }
    }

    nextPage =()=> {
        if(this.state.currentPage<Math.ceil(this.state.totalElements/this.state.usersPerPage)){
            this.getAllStudentUser(this.state.currentPage+1);
        }
    }
        ...
        return (
            <div>
                ...
                <div className="StudentContent">
                    <h2>This is Student content</h2>
                    <Card className={"border border-dark bg-dark text-white"} style={{marginTop:"50px"}}>
                        <Card.Header>List of Student Users</Card.Header>
                            ...
                            <Table bordered hover striped variant="dark">
                                ...
                                <tbody>
                                {
                                    users.length === 0 ?
                                        ...
                                        <tr align="center">
                                            <td colSpan="5"> No Student Users available</td>
                                        </tr> :
                                        users.map((user) => (
                                            <tr key={user.id}>
                                                <td>{user.name}</td>
